...
function CanUpdate()
  ...
  if update_count ~= data.update_count then
...
function RenderParameterButtons()
  ...
  for _, fx_id in ipairs(fx_keys) do
    ...
    for p_id, v in pairs(v.params) do
      ...
      if counter % 5 ~= 0 then reaper.ImGui_SameLine(ctx, 0, ui.item_spacing_x) end
    end

    local popup_id = ("Popup##%d"):format(fx_id)

    if reaper.ImGui_Button(ctx, ("  +  ##%d"):format(fx_id), ui.big_btn_height, ui.big_btn_height) then
...
function RenderLastTouchedButton()
  ...
  if rv and reaper.ImGui_Button(ctx, "  + Last Touched   ", 0, ui.big_btn_height) then
...
function PushMainStyleVars()
  reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_ItemSpacing(), 1, ui.item_spacing_y)
  ...
  reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_FrameRounding(), ui.frame_rounding)
  reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_FrameBorderSize(), ui.frame_border)
  ...
  if visible then
    ...
    if reaper.ImGui_BeginChild(ctx, "Tracks and FX", w, 0, child_flags) then
      if CanUpdate() and not data.lock_track_selection then
        data.track = reaper.GetSelectedTrack2(0, 0, false)
      end

      if data.track then
        local rv, track_name = reaper.GetTrackName(data.track)
        reaper.ImGui_Text(ctx, ("Track: %s"):format(track_name))
        reaper.ImGui_SameLine(ctx, 0, ui.item_spacing_x)
        rv, data.lock_track_selection = reaper.ImGui_Checkbox(ctx, "Lock track selection",
          data.lock_track_selection)
      else
